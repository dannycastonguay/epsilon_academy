...
export async function dragon() {
  ...
  alert("Calling story now!");  // Moved out of the switch
  const result = await story(gender, dragonType);  // Moved out of the switch
...
async function story(gender, dragonType) {
  const intro = (await ask(`You wake up as a ${gender} ${dragonType} dragon in your egg.What will you do?[1] Break out of the egg[2] Stay in the egg`)).trim();
  alert("You chose: " + intro);
